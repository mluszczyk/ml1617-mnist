...
def get_convnet_model():
    ...
    model.add(Convolution2D(16, nb_conv, nb_conv, border_mode='same'))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    assert model.output_shape == (None, 34, 34, 16)
    model.add(Convolution2D(16, nb_conv, nb_conv, border_mode='same', subsample=(2, 2)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    assert model.output_shape == (None, 17, 17, 16)
    ...
    model.add(Convolution2D(64, nb_conv, nb_conv, border_mode='same', subsample=(2, 2)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    assert model.output_shape == (None, 5, 5, 64)

    model.add(Convolution2D(128, nb_conv, nb_conv, border_mode='same', subsample=(2, 2)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    assert model.output_shape == (None, 3, 3, 128)

    model.add(Flatten())
    assert model.output_shape == (None, 128 * 3 * 3)
